...
public class TimeZoneTranslator {
	...
	static DateTime shiftTimeZone(DateTime inputDate, int fromOffset, int toOffset) {
		...
		int theYear = inputDate.getYear();
		int theMonth = inputDate.getMonth();
		int theDay = inputDate.getDay();
		
		
		if(targetHour < 0)
		{
			targetHour = Math.floorMod(targetHour, 24);
			theDay--;
			if(theDay < 1)
			{
				theDay = 31;
		...
		DateTime targetDateTime = new DateTime(theYear,
				theMonth,
				theDay,
				targetHour,
				inputDate.getMinute(),
